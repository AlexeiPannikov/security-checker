class DebuggerDetector: Detector {
    ...
    func runDetection() -> DebuggerReport {
        ...
        if result != 0 {
            report.value = nil
            report.evidence.debuggerCheckError = [
                "sysctl failed with result \(result)"
            ]
            return report
        }
        ...
        if (info.kp_proc.p_flag & P_TRACED) != 0 {
            report.value = true
            report.evidence.debuggerDetected = [
                "Debugger is attached (sysctl, P_TRACED)"
            ]
        } else {
            report.value = false
        }
    ...
    func startMonitoring() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 3.0, repeats: true) {
            ...
            if self.lastReport != nil && self.lastReport?.value != report.value {
